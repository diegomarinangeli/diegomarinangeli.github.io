...
The whole News section sits behind a password prompt (`mk01`) for everyone but Diego — unlocking it sets a `sessionStorage` flag (`newsUnlocked`), so it stays unlocked for the rest of that browser tab's session (surviving reloads and trips to project subpages and back) and only re-locks once the tab/browser is closed; see `script.js`'s `NEWS_PASSWORD`/`NEWS_UNLOCK_KEY` check. This is presentation-only (the JSON files are still directly fetchable), not real access control. Visitor "interests" (tech sub-category filter) and per-story read/unread status are stored in that visitor's `localStorage` (`newsInterests`, `newsReadStatus`) — there's no backend, so nothing is aggregated or sent anywhere.
